...
import { startupErrorDetails } from "../src/stores/backend.js";
...
test("a backend failure reaches the error screen with its code and traceback", () => {
    ...
    assert.deepEqual(startupErrorDetails(error, REQUEST_FAILED), {
        kind: "application",
...
test("authentication and abandoned requests are not the error screen's business", () => {
    ...
    assert.equal(startupErrorDetails(expired, REQUEST_FAILED), null);
    assert.equal(startupErrorDetails(abandoned, REQUEST_FAILED), null);
});
